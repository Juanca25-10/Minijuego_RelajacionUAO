// fireflies.js - versión reparada: sprites, parpadeo, sonidos, net animada, posiciones absolutas
const swingNet = window.swingNet;
const initNet3D = window.initNet3D;
...
// Rutas de assets (según tu estructura)
...
class GameAudio {
    constructor() {
        ...
        // VOLUMENES NUEVOS SEGÚN LO PEDIDO
        this.bg.loop = true;
        this.bg.volume = 0.90;   // ⬆ Aumentado un poco más
        this.net.volume = 1.0;   // ⬆ Máximo
        this.capture.volume = 0.05; // ⬇ Disminuido bastante
    }

    startBg() {
        this.bg.play().catch(e => {/* no forzamos */});
    }
    stopBg() {
        this.bg.pause(); this.bg.currentTime = 0;
    }
    playNet() {
...
class FireflyGame {
    ...
    async _preloadAssets(){
        try {
            ...
            this.netImage = this.assets.getImage("net");
        } catch(e) {
            console.warn("Algunas imágenes no se cargaron:", e);
        }
    }
    ...
    // 🔥 Reproduce sonido y animación 3D de la red
    this.audio.playNet();
    swingNet(); // ← animación del modelo 3D
    ...
    // -------------------------
    // ✔ CONTADOR NUEVO: SOLO NÚMERO GRANDE
    // -------------------------
    drawUI() {
    ...
    // Nueva posición → ARRIBA A LA IZQUIERDA
    const x = 40;
    const y = 35;

    // SOMBRA suave para que se vea bien
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    toggleBtn.addEventListener("click", async () => {
        ...
        if (isActivating) {
            ...
            if (ctaBar) {
                if (window.scrollY > 100) ctaBar.classList.remove("cta-hidden");
            }
        }
    });
});