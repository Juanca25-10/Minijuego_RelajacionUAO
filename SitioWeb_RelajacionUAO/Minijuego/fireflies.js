// fireflies.js — versión corregida sin fondo oscuro
...
class GameAudio {
    constructor() {
        // Asegúrate de que estos elementos estén en tu HTML si los usas
...
        this.capture = document.getElementById("capture-sound");
        ...
        if (this.bg) this.bg.volume = 0.7;
        if (this.net) this.net.volume = 1;
        if (this.capture) this.capture.volume = 0.4;
    }
...
    // ELIMINADO: const darkOverlay = document.getElementById("forest-dark-mode");
...
    if (!toggleBtn) {
        console.error("Error: El botón de inicio con ID 'firefly-toggle-btn' no se encontró.");
        return; 
    }
...
        if(isActivating){
            // ELIMINADO: darkOverlay.classList.add("active");

            if (iconStart) iconStart.classList.add("hidden");
            if (iconClose) iconClose.classList.remove("hidden");
            if (tooltip) tooltip.textContent="Detener Descanso Activo";

            // Si ctaBar existe, se le añade la clase cta-hidden
...
            if (iconStart) iconStart.src = ASSET_IMG + "Button_NoInteractua.png";
            if (tooltip) tooltip.textContent="Iniciar Descanso Activo";

            // Mostrar ctaBar solo si se ha hecho scroll
